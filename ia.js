/* =============================================================================
   ...
   Nada de esto toca la ficha PDF ni el plano 2D: ahí sigue el tipo estándar.
...
/* ------------------------------ estado -------------------------------- */
let lote = null, historial = [], ocupado = false;
...
function aplicar(n, val){
  window.__CASA_IA = { lote:n, bloques:val.ref, construida:val.construida, cap:val.cap };
  if(!R3D.activo()){ const b=document.getElementById("b3d"); if(b) b.click(); }
  R3D.casa(n,null,null);
}
function quitar(){ if(window.__CASA_IA){ delete window.__CASA_IA; if(R3D.activo()&&lote!=null) R3D.casa(lote,null,null); } }
...
function pintar(){
  panel.innerHTML =
    ...
    '<div class="pie">'+tt("Sólo asesores. Las áreas y el 30 % los calcula el motor del sitio, no la IA. La ficha PDF sigue mostrando el tipo estándar.",
                            "Advisors only. Areas and the 30 % rule are computed by the site engine, not by the AI. The PDF sheet still shows the standard type.")+'</div>';
  ...
  else burbuja("err", tt("Este lote se implanta en bancales (47 a 53, 48, 65, 83, 84, 86): el configurador v1 sólo cubre lotes de una plataforma.",
                         "This lot is terraced: configurator v1 only covers single-platform lots."));
}
